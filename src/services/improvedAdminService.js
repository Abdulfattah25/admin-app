...
export class AdminService {
  ...
  static async getUsersByApp(appName = null) {
    ...
    try {
      ...
      // If some rows have missing names, try to enrich from admin_profiles
      ...
      if (userIds.length > 0) {
        try {
          ...
          if (!profErr && profiles) {
            ...
            return rows.map((r) => ({
              ...r,
              // prefer the app-specific name; fallback to profile name if missing
              name: r.name && r.name.trim() !== '' ? r.name : (nameById.get(r.user_id) ?? null),
              // keep existing email, fallback to profile email if missing
              email:
                r.email && r.email.trim() !== '' ? r.email : (emailById.get(r.user_id) ?? null),
            }))
          }
        } catch (e) {
          // Non-fatal; just return raw rows
  ...
  static async getLicensesByApp(appName = null) {
    const appKey = appCacheKey(appName)
    const cacheKey = `licenses:${appKey}`
    const cached = cache.get(cacheKey)
    if (cached) return cached
  ...
  static async generateLicenses(appName, count) {
    ...
    if (error) throw error
    // Invalidate license caches for this app
    const appKey = appCacheKey(appName)
  ...
  static async deleteLicense(licenseId) {
    // Fetch license to know its app for targeted invalidation
